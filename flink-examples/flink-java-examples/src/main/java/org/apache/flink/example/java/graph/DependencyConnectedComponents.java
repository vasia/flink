...
public class DependencyConnectedComponents implements ProgramDescription {
	...
	public static void main(String... args) throws Exception {
		...
		// calculate the common dependency edges
		DataSet<Tuple2<Long, Long>> commonDependencies  = edges.join(edges).where(1).equalTo(1)
				.with(new FlatJoinFunction<Tuple2<Long,Long>, Tuple2<Long,Long>, 
						Tuple2<Long, Long>>() {
							...
							public void join(Tuple2<Long, Long> first, Tuple2<Long, Long> second,
								...
								if (first.f0 != second.f0) {
									out.collect(new Tuple2<Long, Long>(first.f0, second.f0));
								}
							}
				});
		...
		DataSet<Tuple2<Long, Long>> candidates = iteration.getWorkset().join(commonDependencies)
				.where(0).equalTo(0).projectSecond(1).types(Long.class)
				.join(iteration.getSolutionSet()).where(0).equalTo(0)
				.projectSecond(0, 1).types(Long.class, Long.class).distinct()
				.union(iteration.getWorkset()).distinct();
		
//		candidates.print();
		
		DataSet<Tuple2<Long, Long>> changes = candidates.join(edges).where(0).equalTo(0)
