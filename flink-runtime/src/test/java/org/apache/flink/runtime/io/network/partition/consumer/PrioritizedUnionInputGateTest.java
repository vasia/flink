...
public class PrioritizedUnionInputGateTest {
	...
	public void testBasicGetNextLogic() throws Exception {
		...
		final PriorityInputGateWrapper ig1 = new PriorityInputGateWrapper(0,
			new SingleInputGate(testTaskName, new JobID(),
			new IntermediateDataSetID(), ResultPartitionType.PIPELINED,
			0, 3,
			mock(TaskActions.class),
			UnregisteredMetricGroups.createUnregisteredTaskMetricGroup().getIOMetricGroup(),
			true));
		final PriorityInputGateWrapper ig2 = new PriorityInputGateWrapper(1, 
			new SingleInputGate(testTaskName, new JobID(),
			new IntermediateDataSetID(), ResultPartitionType.PIPELINED,
			0, 5,
			mock(TaskActions.class),
		...
		final PrioritizedUnionInputGate union = new PrioritizedUnionInputGate(new InputGate[]{ig1, ig2});

		assertEquals(ig1.getNumberOfInputChannels() + ig2.getNumberOfInputChannels(), union.getNumberOfInputChannels());

		final TestInputChannel[][] inputChannels = new TestInputChannel[][]{
			TestInputChannel.createInputChannels(ig1.getWrappedInputGate(), 3),
			TestInputChannel.createInputChannels(ig2.getWrappedInputGate(), 5)
		};
		...
		((SingleInputGate) ig1.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[0][0].getInputChannel());
		((SingleInputGate) ig1.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[0][1].getInputChannel());
		((SingleInputGate) ig1.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[0][2].getInputChannel());
		((SingleInputGate) ig2.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[1][0].getInputChannel());
		((SingleInputGate) ig2.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[1][1].getInputChannel());
		((SingleInputGate) ig2.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[1][2].getInputChannel());
		((SingleInputGate) ig2.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[1][3].getInputChannel());
		((SingleInputGate) ig2.getWrappedInputGate()).notifyChannelNonEmpty(inputChannels[1][4].getInputChannel());
		...
		verifyBufferOrEvent(union, false, 2, false); // gate 1, channel 2
